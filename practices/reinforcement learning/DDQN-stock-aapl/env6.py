...
class TradingSPYEnv(gym.Env):
    ...
    def __init__(self, train_data_path='historySPY.csv', sma_len=[5], init_invest=10000, gamma=0.98,
        ...
        self.current_step = 2* (self.max_sma_len -1) # minimum episode place for starting
...
#        self.features.reset_index(drop=True,inplace=True)
    ...
    def _get_observation(self):
        observation = []
#        observation.append(self.features['State'].loc[self.current_step-self.max_sma_len+1:self.current_step].to_numpy('float32')) # trailing info
...
#        observation.append(self.features['daily_return'].loc[self.current_step-self.max_sma_len+1:self.current_step].to_numpy('float32'))
        ...
        for col in self.features.columns:
            if 'breakout_' in col:
                observation.append(self.features[col].loc[self.current_step:self.current_step].values) # spot info
#                observation.append(self.features[col].loc[self.current_step-self.max_sma_len+1:self.current_step].values) # Trailing information
        observation = np.append(self.features['State'].loc[self.current_step:self.current_step].to_numpy('float32'), observation) # spot info
    ...
    def reset(self, current_step = None):
        ...
        # add MDD            
        MDD_window = [5,30,60]
        for window in MDD_window:
            rolling_max = self.features['Close'].rolling(window,min_periods=1).max()
            MDD = self.features['Close']/rolling_max -1.0 
            self.features['MDD_'+str(window)] = MDD
