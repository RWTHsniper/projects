...
, std::vector<correlation>& arg_correlation_vec, std::vector<Constraint>& arg_constraint_vec){
    ...
    // x.resize(std::vector<size_t>{Nt+1, num_sv, num_paths},static_cast<double>(0.0),use_bound_check); 
    ...
    // Initialization forBrownian motion
...
void SDE::print_result(){
    ...
    for (size_t j=0; j<num_sv; j++){
        ...
        std::cout << "mean: " << x.mean(Nt, j, 0, Nt, j, num_paths) << " ";
        std::cout << "std: " << x.std(Nt, j, 0, Nt, j, num_paths);
        std::cout << std::endl;
    }
