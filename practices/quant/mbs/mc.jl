...
for i in 1:num_paths
    for j in 2:num_steps
        int_0_u_r[i,j] = ni.integrate(t_sim[1:j], r[i,1:j])
        int_0_u_h[i,j] = ni.integrate(t_sim[1:j], h[i,1:j])
    end
end
...
for i in 1:num_paths # looping over each path
    ...
    int_0_u_r_u = int_0_u_r[i,p_step]
    if p_step < num_steps # prepayment occured
        discount = exp(-int_0_u_r_u)
        M0[i] = discount*get_P_no_prepay(P0,m,t_sim[p_step],T)
    end
    # compute cashflow from coupon
    # compute discounted c
    for (j, t) in enumerate(t_sim[1:p_step]) # j-time step
        discount = exp(-int_0_u_r_u)
        buffer[j] = discount
    end
    M0[i] += c * ni.integrate(t_sim[1:p_step], buffer[1:p_step])
end
