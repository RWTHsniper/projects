...
{
MPI_Irecv(sm2, mat_size*mat_size, MPI_FLOAT, myrank+1, tag, MPI_COMM_WORLD, &request);
MPI_Send(sm1, mat_size*mat_size, MPI_FLOAT, myrank+1, tag, MPI_COMM_WORLD);
for (i=0;i<mat_size;i++)
{	for (j=0;j<mat_size;j++)
{ 
...
{
MPI_Irecv(sm2, mat_size*mat_size, MPI_FLOAT, myrank+2, tag, MPI_COMM_WORLD, &request);
MPI_Send(sm1, mat_size*mat_size, MPI_FLOAT, myrank+2, tag, MPI_COMM_WORLD);
for (i=0;i<mat_size;i++)
{	for (j=0;j<mat_size;j++)
{ 
sm1[i][j]=sm1[i][j]+sm2[i][j];
