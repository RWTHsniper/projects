...
function [t,h1,h2]=CascadeSimulator(x0,sigma_x,sigma_y,tend,u,T)
...
%System equations
%ode = @(t,x) A * x + B*u(t) + sigma_x.* randn(size(x));
R_1 = 1;
R_2 = 1;
A_1 = 2;
A_2 = 5;
ode = @(t,x) [(u(t)-R_1*sqrt(x(1)))/A_1 ;(R_1*sqrt(x(1)) - R_2*sqrt(x(2)))/A_2] + sigma_x.* randn(size(x));
