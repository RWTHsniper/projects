...
This code also considers homogenized strain
...
#            print "summation of sigma "
        F_avg=np.array([[avg_F[0],avg_F[1]],[avg_F[2],avg_F[3]]],dtype=np.float)
        eig_w, eig_v = np.linalg.eig(F_avg)
        file.writelines(str(float(max(eig_w)/counter))+" "+str(avg_sig[0]/counter)+"\n") 
file.close()           
